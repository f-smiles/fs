...
const revealedLengthRef = useRef(0); // in px
  // const POINT_COUNT = 1000;
const POINTS_PER_PIXEL = 0.35; 
useEffect(() => {
...
function findClosestPointIndex(points, x, y) {
  ...
  for (let i = 0; i < points.length; i++) {
    const dx = points[i].current.x - x;
    const dy = points[i].current.y - y;
    const d = dx * dx + dy * dy;
...
function generateBasePoints() {
  ...
  const TOTAL_HEIGHT = documentHeightRef.current;

  const POINTS_PER_PIXEL = 0.35; // density control
  const POINT_COUNT = Math.floor(TOTAL_HEIGHT * POINTS_PER_PIXEL);
  ...
  for (let i = 0; i < POINT_COUNT; i++) {
    ...
    const s = sCurve((y / height) % 1);
    const x = CENTER_X + s * AMPLITUDE;
...
function applyHoverWrap(points) {
  const mouse = mouseRef.current;

  const RADIUS = 140;
  const STRENGTH = 1.15;
  const SWIRL = 1.0;
  const CURVE_BOOST = 1.6;
  ...
  points.forEach((p) => {
    ...
    const px = -dy / dist * SWIRL;
    const py =  dx / dist * SWIRL;

    p.hoverForce.x += px * falloff * STRENGTH * CURVE_BOOST;
    p.hoverForce.y += py * falloff * STRENGTH * CURVE_BOOST;

    p.hoverForce.x += -dx * falloff * COMPRESSION * 0.02;
    p.hoverForce.y += -dy * falloff * COMPRESSION * 0.02;
  });
}
function applyMouseWrap(points) {
  const mouse = mouseRef.current;

  const RADIUS = 140;
  const UNWRAP_RADIUS = 180;
  const WRAP_STRENGTH = 0.018;
  const LOCK_RADIUS = 0.5; // % of radius before locking
  ...
  points.forEach((p) => {
    ...
    if (dist > UNWRAP_RADIUS) {
      p.angularTarget *= 0.9;
      return;
    }


    if (p.wrapped && p.wrapAnchorIndex !== null) {
      const anchor = points[p.wrapAnchorIndex];
      const ox = p.current.x - anchor.current.x;
      const oy = p.current.y - anchor.current.y;

      p.velocity.x += -p.angular * oy * 0.02;
      p.velocity.y +=  p.angular * ox * 0.02;
    ...
    if (dist < RADIUS) {
      ...
      const pull = falloff * WRAP_STRENGTH;

      p.angularTarget += pull;

      if (dist < RADIUS * LOCK_RADIUS && Math.abs(p.angular) > 0.02) {
        p.wrapped = true;
        p.wrapAnchorIndex = findClosestPointIndex(
          points,
          p.current.x,
          p.current.y
        );
    ...
    p.velocity.x += -p.angular * dy * 0.02;
    p.velocity.y +=  p.angular * dx * 0.02;
  });
}
function updatePoints(points) {
  ...
  points.forEach((p) => {
    ...
    const fx = (p.target.x - p.current.x) * stiffness;
    const fy = (p.target.y - p.current.y) * stiffness;

    p.velocity.x = (p.velocity.x + fx) * damping;
    p.velocity.y = (p.velocity.y + fy) * damping;

    const ANGULAR_RESPONSE = 0.06;
    const ANGULAR_DAMPING = 0.9;

    p.angular += (p.angularTarget - p.angular) * ANGULAR_RESPONSE;
    p.angularTarget *= 0.88;
    p.angular *= ANGULAR_DAMPING;

    const HOVER_RESPONSE = 0.18;  
    const HOVER_DAMPING  = 0.88;

    p.velocity.x += p.hoverForce.x * HOVER_RESPONSE;
    p.velocity.y += p.hoverForce.y * HOVER_RESPONSE;

    p.hoverForce.x *= HOVER_DAMPING;
    p.hoverForce.y *= HOVER_DAMPING;

    if (p.wrapped && p.wrapAnchorIndex !== null) {
      const anchor = points[p.wrapAnchorIndex];
      const dx = p.current.x - anchor.current.x;
      const dy = p.current.y - anchor.current.y;
      const dist = Math.sqrt(dx * dx + dy * dy);

      if (dist > 200) {
        p.wrapped = false;
        p.wrapAnchorIndex = null;
  ...
  for (let i = 1; i < points.length - 1; i++) {
    ...
    curr.velocity.x +=
      ((prev.current.x + next.current.x) / 2 - curr.current.x) * spread;
    curr.velocity.y +=
      ((prev.current.y + next.current.y) / 2 - curr.current.y) * spread;
  }
...
function buildPath(points, visibleCount) {
  ...
  const ENTRY_X = -40;    
  const ENTRY_Y = first.y;

  let d = `M ${ENTRY_X} ${ENTRY_Y} L ${first.x} ${first.y}`;
  ...
  for (let i = 0; i < visibleCount - 1; i++) {
    ...
    const cp1x = p1.current.x + (p2.current.x - p0.current.x) * tension;
    const cp1y = p1.current.y + (p2.current.y - p0.current.y) * tension;

    const cp2x = p2.current.x - (p3.current.x - p1.current.x) * tension;
    const cp2y = p2.current.y - (p3.current.y - p1.current.y) * tension;

    d += ` C ${cp1x} ${cp1y}, ${cp2x} ${cp2y}, ${p2.current.x} ${p2.current.y}`;
  }
  ...
  function animate() {
    ...
    const visibleCount = Math.floor(
      revealProgressRef.current * points.length
    );

    const activePoints = points.slice(0, visibleCount);

applyHoverWrap(activePoints);
applyMouseWrap(activePoints);
updatePoints(points);

    if (pathRef.current) {
      pathRef.current.setAttribute(
        "d",
        buildPath(points, visibleCount)
      );
    }
...
const lastScrollYRef = useRef(0);
  ...
  useEffect(() => {
  if (!documentHeightRef.current) return;
...
const handleMouseMove = (e) => {
  ...
  const local = pt.matrixTransform(inverted);
...
const handleScroll = () => {
  ...
  const rawStep = delta * SCROLL_DAMPING;

  const step = Math.max(
    -MAX_STEP_PX,
    Math.min(rawStep, MAX_STEP_PX)
  );

  revealedLengthRef.current += step;

const maxLength = documentHeightRef.current;

  revealedLengthRef.current = Math.max(
    0,
    Math.min(revealedLengthRef.current, maxLength)
  );

  revealProgressRef.current =
    revealedLengthRef.current / maxLength;
};
...
<svg
  ref={svgRef}
  viewBox={`0 0 ${viewportRef.current.width} ${documentHeightRef.current || viewportRef.current.height}`}
